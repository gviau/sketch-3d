...
int main(int argc, char** argv) {
    ...
    shader->SetSourceFile("Shaders/FrustumCullingStaticBatching/vert", "Shaders/FrustumCullingStaticBatching/frag");
    ...
    for (size_t i = 0; i < NUM_TEAPOTS_X; i++) {
        for (size_t j = 0; j < NUM_TEAPOTS_Z; j++) {
            ...
            nodes[idx].SetStatic(true);
            idx += 1;
        }
    }

    Renderer::GetInstance()->GetSceneTree().PerformStaticBatching();
