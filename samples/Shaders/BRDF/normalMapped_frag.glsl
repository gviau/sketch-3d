...
void main() {
	...
	vec3 B = normalize(cross(tangent, normal));
	mat3 tangentToWorld = mat3(T, B, N);
	N = tangentToWorld * normalize(texture(normalTexture, uv).rgb * 2.0 - 1.0);
...
vec3 textbookBlinnPhong(vec3 N, vec3 L, vec3 H, vec3 diffuse_color, vec3 specular_color) {
	vec3 albedo = pow( texture(diffuseTexture, uv).rgb, vec3(2.2) );
	float lambert = max(dot(N, L), 0.0);
	vec3 diffuse = lambert * diffuse_color * albedo;
...
vec3 blinnPhong(vec3 N, vec3 L, vec3 H, vec3 diffuse_color, vec3 specular_color) {
	vec3 albedo = pow( texture(diffuseTexture, uv).rgb, vec3(2.2) );
	vec3 diffuse = diffuse_color * albedo / 3.14159;
...
vec3 cookTorrance(vec3 N, vec3 L, vec3 H, vec3 V, vec3 diffuse_color, vec3 specular_color) {
	...
	return (diffuse + specular) * nDotL * albedo;
}